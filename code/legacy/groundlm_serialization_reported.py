        ...
        for task in TASKS:
            ...
            for qinfo in gt_queries:
                ...
                for dtype in DISTRACTOR_TYPES:
                    ...
                    gt_dist = compute_ground_truth(G_dist, task)
                    gt_match = next((g for g in gt_dist if g.get("source_nodes") == source_nodes and g.get("query") == query), None)
                    if gt_match and gt_match.get("answer") != answer:
...
def compute_format_stability(results: List[Dict]) -> Dict:
    ...
    for key, group in groups.items():
        ...
        for fmt_a, fmt_b in format_pairs:
            if fmt_a in answers and fmt_b in answers:
                agree = 1.0 if answers[fmt_a] == answers[fmt_b] else 0.0
                agreements.append(agree)
                per_format_pair[(fmt_a, fmt_b)].append(agree)

        # Overall agreement across all formats for this instance
    ...
    fs_per_task = {t: np.mean(v) for t, v in per_task.items()}
...
def compute_order_sensitivity(results: List[Dict]) -> Dict:
    ...
    for key, group in groups.items():
        ...
        for r in group:
            extracted = extract_answer(r["model_answer"], r["task"])
            flipped = 1.0 if extracted != canonical_answer and extracted != "unclear" else 0.0
